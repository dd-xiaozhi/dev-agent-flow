...
> **业务可验证的标准**，每条对应一个测试场景。
>
> **编号规则**：AC-001 起递增。契约**冻结（frozen）后**编号永不复用，删除标 `[DELETED]` 保留编号（下游 spec / 测试 / 子任务可能已引用编号）；**冻结前**（draft / review 评审期）的 AC 增删**直接物理删除并重排**，不留 `[DELETED]` 墓碑（编号尚未被下游引用，墓碑是噪音）。
...
**只列有待确认项的角色子表;无 TBD 的角色不显示(禁止空表格);全部角色都无则整章删除、后续章节顺次重编号,不保留空章节。** 每个 TBD 必含"背景 + 建议答案"。已裁决归档的 TBD 溯源写变更日志,不留在本章。
...
- ❌ 不写"对 planner / generator 的指引"类章节——下游角色所需的技术背景放 `source/` 调研素材，契约只写业务应然（共识文档面向 PM/QA 评审，技术交接走 artifact 通道）
