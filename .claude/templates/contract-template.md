...
> 本模板由 **doc-librarian** agent 填充。完成后置于 `.chatlabs/task/store/<story_id>/contract.md`。
>
> **设计原则**：**需求明确，无法确认的点一定标明**。未明确的部分标 `TBD`，不要臆造。有任何疑问一定要提出来

---

## 使用说明

1. 复制本模板为 `.chatlabs/task/store/<story_id>/contract.md`
...
> **拆分原则：在进行  AC 拆分时不要拆的过细，比如简单的参数校验拆分成多个，这就不太对了，如果参数校验逻辑简单，直接一个 AC 即可，复杂的逻辑才需要拆分**
...
**期望**：
- 返回 201
- 响应体符合 contract.md 接口契约中 `XxxCreateResponse` schema
