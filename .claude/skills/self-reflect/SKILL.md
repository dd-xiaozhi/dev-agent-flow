...
1. 根据 `trigger` 确定要读哪些文件：
   ...
   - `tapd-reopen` → 读 `meta.json`（含 summary 字段）+ `blockers.md`（exists 兜底,不存在视为无 blocker）
   - `workflow-review` → 读近期（近 30 天）所有 flow-log
   - `blocker` → 读当前 blockers.md（exists 兜底）
   - `manual` → 无特定要求
...
2. **存储成功模式**（若平均分 ≥ 8）：
   ...
   if avg_score >= 8:
       ltm.store(
           key=f"success-pattern:{context_ref}",
           content={
               "pattern": summary,
               "trigger": trigger,
