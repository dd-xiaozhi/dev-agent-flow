...
1. **判定 @ 角色集**(按上表)→ 读 `env.yaml.tapd.team_roles.<role>`,每项取中文名(`"许迪智(DDXu)"` → `许迪智`);若 @ 对象 == 通知发起人本人,从列表剔除(自己不 @ 自己)
2. **名字 → 手机号**:在 `env.yaml.notify.qiwei_mentions`(扁平 `[{name, mobile}]`,该文件已 gitignore 不入仓)按 `name` 匹配取 `mobile`,组成 `mentioned_mobile_list`
3. **发 markdown 主消息**(富文本内容,所有类型必发)
4. **mobile 非空** → 紧跟发一条 **text** 消息(一句话待办 + `mentioned_mobile_list`);**全空** → 跳过 @ 并 WARN:`@ 对象(<名字列表>)在 notify.qiwei_mentions 的 mobile 未填,跳过 @,请在 env.yaml 补手机号`(不阻断流程)
...
> 通讯录:`env.yaml.notify.qiwei_mentions` = 扁平 `[{name, mobile}]`(已 gitignore,不入仓)。角色成员名取自 `tapd.team_roles.<role>`,按 `name` 在通讯录查 `mobile`。发送取 `mobile`,全空则只发 markdown 不 @。
