...
# 只跑指定 rule
bash scripts/fitness-run.sh layer-boundary
bash scripts/fitness-run.sh openapi-lint
bash scripts/fitness-run.sh layer-boundary openapi-lint
```
...
- 脚本：`fitness/*.sh`
- 项目特定规范（渐进式披露入口）：`.chatlabs/knowledge/README.md`（获取 `<layer>/<module>/fitness-rules.md` 路径）
