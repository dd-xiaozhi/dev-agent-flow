...
- 读取 `.chatlabs/knowledge/INDEX.md` 获取当前项目的 API 规范路径（如 `backend/api-conventions.md`），按该文件执行；不存在时 fallback 到 `docs/` 并提示团队运行 `/init-project`
...
- 项目特定规范（渐进式披露入口）：`.chatlabs/knowledge/INDEX.md`（从中获取 `contract/`、`product/` 等模块的规范路径）
