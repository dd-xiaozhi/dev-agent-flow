...
> ChatLabs 后端团队 AI 开发工作流 harness。一套 Claude Code 配置（`.claude/`）+ 规范文档（`docs/`）的组合，定义从产品需求到代码交付的全流程。
...
| `.claude/commands/` | 13 个 slash command | 用户手动调用 |
...
| `.claude/scripts/` | harness 内部工具（gc.py / gc-run.sh / contract-drift-check.py / paths.py） | 被 skill / agent / hook 调用 |
...
| [`docs/coding-convention.md`](docs/coding-convention.md) | 后端编码规范（Java / 命名 / 测试） |
...
| `/tapd-consensus-push` / `fetch` | 契约评审双向同步 |
| `/tapd-subtask-emit` / `close` / `reopen` | 子任务派发与状态机 |
| `/sprint-review` | 即时复盘 |
