...
| 模式 | 用途 |
|------|------|
| `.chatlabs/task/store/<story_id>/contract.md` | doc-librarian 产出 |
| `.chatlabs/task/store/<story_id>/spec.md` | planner 产出 |
...
```bash
# 占位符变体扫描（一键自查）
grep -rE "(<[a-z]+-[a-z]+>|\{[a-z_]+\})" --include='*.md' .claude/ .chatlabs/knowledge/
```
