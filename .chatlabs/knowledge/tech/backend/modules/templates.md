...
产物模板与流程模板的存放处，是 Flow 的**数据化骨架**——改流程 = 改 JSON / Markdown，不改代码。
...
| `evaluator-rubric.md` | 模板 | evaluator 验收 rubric 骨架 |
| `spec.md` | 模板 | planner 写 spec.md 的骨架 |
| `flows/` | 数据 | flow 编排定义（tapd-full / local-spec / local-plan / local-vibe） |
| `story/` | 模板 | story 目录骨架 |
| `task-report/` | 模板 | task 报告三件套骨架（meta.json / audit.jsonl / blockers.md） |
...
├── evaluator-rubric.md          evaluator 验收
├── spec.md                      planner 用
├── flows/                       4 个 flow 模板（JSON）
│   ├── tapd-full.json
│   ├── local-spec.json
│   ├── local-plan.json
│   └── local-vibe.json
├── story/                       story 目录骨架
└── task-report/                 task 报告骨架
```
...
| `skill` | 调用 skill（git-commit-push / jenkins-deploy / tapd-pull） |
| `command` | 调用斜杠命令（/tapd-consensus-push / /sprint-review） |
